...
#   FRONTEND_TAG       Frontend image tag (default: ${VERSION}-prod)
#   OPENAI_API_KEY     Secret passed to backend (required)
#   ALLOWED_IPS        Comma-separated IPs for backend access (required)
...
# Auto-version when VERSION not provided: vYYYY.MM.DD-HHMM
if [[ -z "${VERSION:-}" ]]; then
  VERSION="v$(date +%Y.%m.%d-%H%M)"
fi
BACKEND_TAG=${BACKEND_TAG:-${VERSION}}
FRONTEND_TAG=${FRONTEND_TAG:-${VERSION}-prod}

HELM_VALUES="${ROOT_DIR}/deploy/helm/values-prod.yaml"
CHART_PATH="${ROOT_DIR}/deploy/helm/chatapp"
RELEASE_NAME="chatapp-prod"
NAMESPACE="chat"
...
if [[ -z "${OPENAI_API_KEY:-}" ]]; then
  echo "[err] OPENAI_API_KEY is required for prod" >&2; exit 2
fi
if [[ -z "${ALLOWED_IPS:-}" && -n "${ALLOWED_IPS_DEFAULT:-}" ]]; then
  ALLOWED_IPS="${ALLOWED_IPS_DEFAULT}"
fi
if [[ -z "${ALLOWED_IPS:-}" ]]; then
  echo "[err] ALLOWED_IPS is required for prod (or set ALLOWED_IPS_DEFAULT in scripts/deploy.env)" >&2; exit 2
fi
...
HELM_SET=(
  ...
  "--set-string" "backend.secretEnv.OPENAI_API_KEY=${OPENAI_API_KEY}"
  "--set-string" "backend.secretEnv.ALLOWED_IPS=${ALLOWED_IPS_ESCAPED}"
)
...
${KUBECTL_BIN} rollout status deploy/${RELEASE_NAME}-chatapp-backend -n "${NAMESPACE}" --timeout=180s || true
${KUBECTL_BIN} rollout status deploy/${RELEASE_NAME}-chatapp-frontend -n "${NAMESPACE}" --timeout=180s || true
...
echo "[ok] Done. Add to /etc/hosts if needed: '127.0.0.1 chat.garden' and test: curl http://chat.garden/api/health"
